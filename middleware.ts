...
async function checkAuthStatus(request: NextRequest): Promise<boolean> {
  ...
  try {
    ...
    console.error(
      '❌ [AUTH CHECK] Auth status check error:',
...
export async function middleware(request: NextRequest) {
  ...
  if (
    ...
    console.log(
      '⏭️ [MIDDLEWARE] Skipping middleware for static/API route:',
      pathname
    );
  ...
  if (isAuthRoute && isAuthenticated) {
    console.log(
      '🔄 [MIDDLEWARE] Redirecting authenticated user away from auth page to /'
    );
