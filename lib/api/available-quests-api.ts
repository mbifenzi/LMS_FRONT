...
import { AvailableQuestsResponse, ApiQuest } from "@/types/types";
...
export async function fetchAvailableQuests(): Promise<AvailableQuestsResponse> {
  ...
  try {
    ...
    return {
      ...
      total_count: data.total_count ?? ((data.available_quests as ApiQuest[])?.length || 0),
    };
  } catch (error) {
...
export async function fetchAvailableQuestById(id: string): Promise<ApiQuest | null> {
  const baseUrl = API_URL;
