      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          {visible.map((quest) => (
            <div
              key={quest.id}
              className="flex flex-col justify-between p-3 rounded-lg border bg-card hover:bg-accent/50 dark:hover:bg-old-background transition-colors min-h-[100px] dark:bg-black"
            >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <Link
                        ...
                        className="font-medium text-blue-600 text-sm truncate hover:underline"
                      >
                        {quest.name}
                      </Link>
                      {quest.difficulty && (
                        <span className="px-1.5 py-0.5 text-[10px] rounded-md border text-muted-foreground flex-shrink-0">
                          {quest.difficulty}
                        </span>
                      )}
                    </div>
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        ...
        <Link
          href="/quest-catalog"
          className="text-blue-500 hover:text-blue-600 text-sm font-medium transition-colors hover:underline focus:outline-none focus:underline"
        >
          View all
        </Link>
      </div>
