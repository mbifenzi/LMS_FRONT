...
export default function CourseCard({ course, variant = "catalog" }: CourseCardProps) {
  ...
  if (variant === "dashboard") {
    return (
      <Link href={`/course-catalog/${course.id}`} className="block">
  ...
  // Catalog variant
  return (
    <Link href={`/course-catalog/${course.id}`} className="block group">
      <div className="border rounded-lg overflow-hidden bg-card shadow-sm transition-all duration-200 group-hover:shadow-lg group-hover:-translate-y-1">
        {/* Course Image */}
