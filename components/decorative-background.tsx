...
export function DecorativeBackground({
  ...
  const sideClasses =
    side === 'left'
      ? 'lg:inset-y-0 lg:left-0 lg:right-1/2'
      : 'lg:inset-y-0 lg:right-0 lg:left-1/2';
    ...
    switch (variant) {
      case 'purple':
        return {
          ...
          square:
            'from-purple-100/40 to-violet-100/40 dark:from-purple-500/10 dark:to-violet-500/10',
      ...
      case 'green':
        return {
          ...
          square:
            'from-green-100/40 to-emerald-100/40 dark:from-green-500/10 dark:to-emerald-500/10',
          dashed: 'border-slate-300/50 dark:border-slate-600/30',
      ...
      default: // blue
        return {
          ...
          square:
            'from-blue-100/40 to-indigo-100/40 dark:from-blue-500/10 dark:to-indigo-500/10',
          dashed: 'border-slate-300/50 dark:border-slate-600/30',
  ...
  return (
    <div
      className={`absolute inset-0 ${sideClasses} pointer-events-none overflow-hidden`}
    >
      {/* Geometric shapes */}
      <div
        className={`absolute top-20 left-16 h-32 w-32 border ${colors.circle} rounded-full`}
      ></div>
      <div
        className={`absolute top-40 right-20 h-24 w-24 bg-gradient-to-br ${colors.square} rotate-12 rounded-lg`}
      ></div>
      <div
        className={`absolute bottom-32 left-20 h-20 w-20 border-2 border-dashed ${colors.dashed} rotate-45 rounded-xl`}
      ></div>
      ...
      >
        <path
          d="M-50 100 Q100 150 150 200 T300 300 Q350 350 300 400 T150 500 Q100 550 200 600"
        ...
        />
        <path
          d="M-20 200 Q80 250 130 300 T270 400 Q320 450 270 500 T130 600 Q80 650 180 700"
      ...
      <div className="absolute top-1/4 left-1/4 grid grid-cols-6 gap-4 opacity-20">
        {Array.from({ length: 24 }).map((_, i) => (
          <div key={i} className={`h-1 w-1 ${colors.dots} rounded-full`}></div>
        ))}
      </div>

      {/* Additional geometric elements */}
      <div
        className={`absolute right-16 bottom-20 h-16 w-16 border-t-4 border-l-4 ${colors.corner}`}
      ></div>
      <div
        className={`absolute top-1/3 left-8 h-40 w-8 bg-gradient-to-b ${colors.line} -rotate-12 transform`}
      ></div>
    </div>
  );
}
